# Builds OFFICE_PAYCHECKS for the Office Staff > Weekly Checks grid.
# Reads incoming-freightiq/: SF+J&A paycheck history (W-2 loaded),
# J&A contractor payments (QB) + VendorEmployeePayments (Chase) for 1099.
# Excludes reimbursements. Hardcoded overrides: Maria Con, Mairena, Logic.
import xlrd, io, json, re, csv, datetime
...
def parse_pc(f):
    sh = xlrd.open_workbook('incoming-freightiq/' + f).sheets()[0]; out = []
...
sf = parse_pc('ShowFreightInc_PaycheckHistory_06242026_1123.xls')
ja = parse_pc('J&A.ManagementGroupLLC_PaycheckHistory_06242026_1124.xls')
...
def add_contractor(dt, raw, amt):
    c = canon(raw)
    if c is None: return
    if c[0] == 'UNMAPPED': print('UNMAPPED', raw); return
...
qb = xlrd.open_workbook('incoming-freightiq/J&A.ManagementGroupLLC_ContractorPayments_06242026_1158.xls').sheets()[0]
...
for row in list(csv.reader(open('incoming-freightiq/VendorEmployeePayments (11).csv', encoding='utf-8-sig')))[1:]:
...
# Mairena Tapias (Jon Marcus assistant), 100% CE, paid as expense
...
data = {'source':'W-2 paychecks (loaded) + contractor payments QB+Chase (dated, excl reimbursements) · Jan 2 – Jun 24, 2026', 'weeks':wlabel, 'sections':out}
io.open('incoming-freightiq/_paycheck_grid.json', 'w', encoding='utf-8').write(json.dumps(data))
