                ...
                <tbody>
                  {PC.sections.map(sec => {
                    ...
                    ...sec.rows.flatMap(r => {
                      ...
                      if (!open) return [main];
                      if (isCon) return [main,
                        sub("commission","#4fc3f7", w=>r.commission&&r.commission[w]),
                        sub("car allowance","#b39ddb", w=>r.car&&r.car[w]),
                        sub("health insurance (co-paid)","#b39ddb", w=>r.health&&r.health[w]),
                        sub("loaded (total cost)","var(--or)", w=>{const s=((r.camts&&r.camts[w])||0)+((r.commission&&r.commission[w])||0)+((r.car&&r.car[w])||0)+((r.health&&r.health[w])||0);return s||null;}),
                      ];
                      return [main,
                        sub("gross","var(--mu)", w=>r.gross&&r.gross[w]),
                        sub("taxes withheld","#ff5252", w=>(r.gross&&r.gross[w]!=null&&r.net&&r.net[w]!=null)?Math.round((r.gross[w]-r.net[w])*100)/100:null),
                        sub("employer cost","#b39ddb", w=>(r.amts&&r.amts[w]!=null&&r.gross&&r.gross[w]!=null)?Math.round((r.amts[w]-r.gross[w])*100)/100:null),
                        sub("loaded (total cost)","var(--or)", w=>r.amts&&r.amts[w]),
                      ];
                    }),
                    <tr key={sec.name+"-t"}>
